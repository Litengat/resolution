...
export async function loadMultipleImagesAsDataUrls(
  paths: string[]
): Promise<string[]> {
  const promises = paths.map((path) => loadImageAsDataUrl(path));
...
const svgUrl = await loadImageAsDataUrl('/path/to/image.svg');
const pngUrl = await loadImageAsDataUrl('/path/to/image.png');
const jpgUrl = await loadImageAsDataUrl('/path/to/image.jpg');

// Convert to base64 with custom options
const base64 = await convertImageToBase64('/path/to/image.jpg', {
  width: 128,
  height: 128,
  outputFormat: 'webp',
  quality: 0.8
...
if (isSupportedImageType('/path/to/image.png')) {
  // Process the image
}
